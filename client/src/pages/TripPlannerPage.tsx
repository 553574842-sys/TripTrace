...
export default function TripPlannerPage(): React.ReactElement | null {
  ...
  const titleCenterKey = useRef<string | null>(null)
  const [titleMapCenter, setTitleMapCenter] = useState<[number, number] | null>(null)
  ...
  useEffect(() => {
    if (!trip?.id || !trip.title?.trim()) return
    const hasGeoPlaces = places.some(p => p.lat != null && p.lng != null)
    if (hasGeoPlaces) {
      setTitleMapCenter(null)
      return
    }

    const query = trip.title.trim()
    const key = `${trip.id}:${query}`
    if (titleCenterKey.current === key) return
    titleCenterKey.current = key

    let cancelled = false
    const compactQuery = query
      .replace(/[\u7b2c\d\u4e00\u4e8c\u4e09\u56db\u4e94\u516d\u4e03\u516b\u4e5d\u5341]+[\u5929\u65e5]|[0-9]+[\u65e5\u5929\u665a\u591c]|\u65c5\u884c|\u65c5\u6e38|\u884c\u7a0b|\u653b\u7565|\u81ea\u7531\u884c|\u5468\u672b|\u8def\u7ebf|\u8ba1\u5212/g, ' ')
      .replace(/\s+/g, ' ')
      .trim()
    const queries = Array.from(new Set([query, compactQuery].filter(Boolean)))

    ;(async () => {
      for (const q of queries) {
        try {
          const data = await mapsApi.search(q, language || 'zh')
          const match = (data.places || []).find((p: any) => {
            const lat = Number(p.lat)
            const lng = Number(p.lng)
            return Number.isFinite(lat) && Number.isFinite(lng)
          })
          if (match && !cancelled) {
            setTitleMapCenter([Number(match.lat), Number(match.lng)])
            return
          }
        } catch {
          // Keep the current default center if title lookup is unavailable.
        }
      }
      if (!cancelled) setTitleMapCenter(null)
    })()

    return () => { cancelled = true }
  }, [trip?.id, trip?.title, places, language])
  ...
  const mapTileUrl = settings.map_tile_url || 'https://webrd01.is.autonavi.com/appmaptile?lang=zh_cn&size=1&scale=1&style=7&x={x}&y={y}&z={z}'
  const defaultCenter = [settings.default_lat || 39.9042, settings.default_lng || 116.4074] as [number, number]
  const defaultZoom = settings.default_zoom || 11
  const hasGeoPlaces = places.some(p => p.lat != null && p.lng != null)
  const mapCenter = !hasGeoPlaces && titleMapCenter ? titleMapCenter : defaultCenter
  const mapZoom = !hasGeoPlaces && titleMapCenter ? Math.max(defaultZoom, 12) : defaultZoom
    ...
    return (
      <div style={{
        ...
        <div style={{ marginBottom: 28 }}>
          <img
            src={document.documentElement.classList.contains('dark') ? '/icons/triptrace-loading-light.gif' : '/icons/triptrace-loading-dark.gif'}
            alt="Loading"
            width={64}
            height={64}
          />
