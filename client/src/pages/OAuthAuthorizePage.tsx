  ...
  async function validateRequest() {
    ...
    try {
      ...
      if (!result.consentRequired) {
        // Consent already on record 鈥?auto-approve silently with the full validated scope
        setPageState('auto_approving')
        await submitConsent(true, result.scopes ?? [])
        return
      }

      // Pre-select all scopes the client is requesting 鈥?user can deselect
        ...
        <div className="min-h-screen flex items-center justify-center p-4" style={{ background: 'var(--bg-primary)' }}>
          <div className="w-full max-w-sm rounded-xl shadow-lg p-8 space-y-5" style={{ background: 'var(--bg-card)' }}>
            <div className="text-center space-y-2">
              ...
              <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>
                <strong>{validation?.client?.name || clientId}</strong> wants access to your TripTrace account. Please sign in first.
              </p>
            </div>
            <button
  ...
  return (
      <div className="min-h-screen flex items-center justify-center p-4" style={{ background: 'var(--bg-primary)' }}>
        <div className="w-full max-w-2xl rounded-xl shadow-lg overflow-hidden flex flex-col sm:flex-row" style={{ background: 'var(--bg-card)' }}>
          ...
          {/* Left panel 鈥?app identity + actions */}
            ...
            <div className="flex-1 space-y-4">
              ...
              <div>
                ...
                <p className="text-sm mt-2" style={{ color: 'var(--text-secondary)' }}>
                  This application is requesting access to your TripTrace account.
                </p>
              </div>
            </div>
          ...
          {/* Right panel 鈥?selectable scopes */}
            ...
            <div className="space-y-6">
              {Object.keys(scopesByGroup).length > 0 && (
                  <div>
                    ...
                    {validation?.scopeSelectable ? (
                        /* DCR client 鈥?user selects which scopes to grant */
                            ...
                            return (
                              ...
                              {groupScopes.filter(s => selectedScopes.includes(s)).length}/{groupScopes.length}
                                  ...
                                  <div className="divide-y" style={{ borderColor: 'var(--border-primary)' }}>
                                    ...
                                    {s.endsWith(':delete') ? '[del]' : s.endsWith(':write') ? '[edit]' : '[read]'}
                                  </span>
                        ...
                        /* Settings-created client 鈥?scopes are fixed, show read-only */
              ...
              {/* Always-available tools 鈥?granted regardless of scopes */}
              <div>
                  ...
                  {[
                    ...
                    { name: 'get_trip_summary', desc: 'Read a trip overview needed to use any other tool' },
                      ...
                      <div key={name} className="flex items-start gap-2.5 px-3 py-2 rounded-lg" style={{ background: 'var(--bg-secondary)' }}>
                        <span className="mt-0.5 text-base leading-none flex-shrink-0">[read]</span>
                        <div className="min-w-0">
