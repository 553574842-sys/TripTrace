...
beforeEach(() => {
  // SharedTripPage does NOT require authentication â€?do NOT seed auth store
  resetAllStores();
  vi.clearAllMocks();
});
...
describe('SharedTripPage', () => {
  ...
  describe('FE-PAGE-SHARED-014: Language picker toggles', () => {
    it('opens language dropdown and closes after selecting a language', async () => {
      ...
      // Dropdown should close â€?EspaÃ±ol is no longer visible
      expect(screen.queryByRole('button', { name: /espaÃ±ol/i })).toBeNull();
    });
  });

  describe('FE-PAGE-SHARED-015: TripTrace branding footer is rendered', () => {
    it('renders the Shared via TripTrace footer', async () => {
      renderSharedTrip('test-token');
