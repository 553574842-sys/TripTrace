...
import { Users, Map, Briefcase, Shield, Trash2, Edit2, FileText, Eye, EyeOff, Save, CheckCircle, XCircle, Loader2, UserPlus, ArrowUpCircle, ExternalLink, Download, Sun, Link2, Copy, Plus, RefreshCw, AlertTriangle, SlidersHorizontal, UserCog, Puzzle, Settings as SettingsIcon, Bell, Database, ScrollText, Bug } from 'lucide-react'
        ...
        <div className="w-full px-4 sm:px-6 lg:px-8 py-8">
          ...
          {demoMode && (
            <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-xl flex items-center justify-between">
              <div>
                <p className="text-sm font-semibold text-amber-900">演示基线</p>
                <p className="text-xs text-amber-700">保存当前状态作为定时重置点。管理员创建的旅行和设置会被保留。</p>
              </div>
              <button
                onClick={async () => {
                  try {
                    await adminApi.saveDemoBaseline()
                    toast.success('演示基线已保存，后续重置会恢复到当前状态。')
                  } catch (e) {
                    toast.error(getApiErrorMessage(e, '保存演示基线失败'))
                  }
                }}
          ...
          {/* Sidebar layout 鈥?nav on the left, active panel on the right */}
          <PageSidebar
                      ...
                      <div key={inv.id} className="px-5 py-3 flex items-center gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-xs text-slate-400 mt-0.5">
                            {inv.used_count}/{inv.max_uses === 0 ? 'unlimited' : inv.max_uses} {t('admin.invite.uses')}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setOidcConfig(c => ({ ...c, client_secret: e.target.value }))}
                        placeholder={oidcConfig.client_secret_set ? 'configured' : ''}
                      className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-slate-400 focus:border-transparent"
                    />
                  </div>
                  <button
          ...
          {activeTab === 'notifications' && (() => {
            ...
            const saveNotifications = async () => {
              // Saves credentials only 鈥?channel activation is auto-saved by the toggle
              ...
              <div className="space-y-4">
                  ...
                  <div className={`p-6 space-y-3 ${!emailActive ? 'opacity-50 pointer-events-none' : ''}`}>
                    {smtpLoaded && [
                      ...
                      { key: 'smtp_port', label: 'SMTP Port', placeholder: '587' },
                        { key: 'smtp_user', label: 'SMTP User', placeholder: 'triptrace@example.com' },
                        { key: 'smtp_pass', label: 'SMTP Password', placeholder: 'configured', type: 'password' },
                        { key: 'smtp_from', label: 'From Address', placeholder: 'triptrace@example.com' },
                    ].map(field => (
                      <div key={field.key}>
                ...
                <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                  ...
                  <div className="p-6 space-y-3">
                    {smtpLoaded && (
                      <div>
                        ...
                        <input
                          type="text"
                          value={smtpValues.admin_webhook_url === 'configured' ? '' : smtpValues.admin_webhook_url || ''}
                          onChange={e => setSmtpValues(prev => ({ ...prev, admin_webhook_url: e.target.value }))}
                          placeholder={smtpValues.admin_webhook_url === 'configured' ? 'configured' : 'https://discord.com/api/webhooks/...'}
                          className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-slate-400 focus:border-transparent"
                        />
                      </div>
                    )}
                  ...
                  <div className="px-6 pb-4 flex items-center gap-2 border-t border-slate-100 pt-4">
                    ...
                    <button
                      onClick={async () => {
                        const url = smtpValues.admin_webhook_url === 'configured' ? undefined : smtpValues.admin_webhook_url
                        if (!url && smtpValues.admin_webhook_url !== 'configured') return
                        try {
                      ...
                      <>
                        ...
                        <div>
                          ...
                          <div className="flex gap-2">
                            <input
                              type="password"
                              value={smtpValues.admin_ntfy_token === 'configured' ? '' : smtpValues.admin_ntfy_token || ''}
                              onChange={e => setSmtpValues(prev => ({ ...prev, admin_ntfy_token: e.target.value }))}
                              placeholder={smtpValues.admin_ntfy_token === 'configured' ? 'configured' : ''}
                            ...
                            />
                            {smtpValues.admin_ntfy_token === 'configured' && (
                              <button
                                onClick={async () => {
                                  try {
                  ...
                  <div className="px-6 pb-4 flex items-center gap-2 border-t border-slate-100 pt-4">
                      ...
                      onClick={async () => {
                        try {
                          await authApi.updateAppSettings({
                            ...
                            ...(smtpValues.admin_ntfy_token && smtpValues.admin_ntfy_token !== 'configured'
                    ...
                    <button
                      onClick={async () => {
                        ...
                        if (!topic) return
                        try {
                          const token = smtpValues.admin_ntfy_token && smtpValues.admin_ntfy_token !== 'configured'
      ...
      >
        <div className="flex gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">A new JWT secret will be generated immediately. Every logged-in user 鈥?including you 鈥?will be signed out and will need to log in again.</p>
          </div>
        </div>
      </Modal>
    </div>
