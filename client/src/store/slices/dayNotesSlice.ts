...
export interface DayNotesSlice {
  ...
  deleteDay: (tripId: number | string, dayId: number | string) => Promise<void>
...
export const createDayNotesSlice = (set: SetState, get: GetState): DayNotesSlice => ({
  ...
  deleteDay: async (tripId, dayId) => {
    const prev = {
      ...
      selectedDayId: get().selectedDayId,
    }
    ...
    set(state => {
      ...
      const nextDays = state.days.filter(d => d.id !== parseInt(key))
      return {
        days: nextDays,
        assignments: nextAssignments,
        dayNotes: nextDayNotes,
        selectedDayId: state.selectedDayId === parseInt(key) ? (nextDays[0]?.id ?? null) : state.selectedDayId,
      }
    })
    try {
      ...
      set(prev)
      throw new Error(getApiErrorMessage(err, 'Error deleting day'))
    }
  },
