// Shared types for the TripTrace travel planner
...
export function getApiErrorMessage(err: unknown, fallback: string): string {
  if (typeof err === 'object' && err !== null && 'response' in err) {
    ...
    if (apiErr.response?.status === 400) return fallback
    if (apiErr.response?.status === 401) return '登录已过期，请重新登录后再试。'
    if (apiErr.response?.status === 403) return '你没有权限执行这个操作。'
    if (apiErr.response?.status === 404) return '没有找到对应的数据，请刷新后再试。'
    if (apiErr.response?.status && apiErr.response.status >= 500) return '服务器处理失败，请检查填写内容或稍后重试。'
  }
  if (err instanceof Error && !/^Request failed with status code \d+$/i.test(err.message)) return err.message
  return fallback
}
