...
apiClient.interceptors.response.use(
    ...
    async (error) => {
      ...
      // HTML body (a JS redirect page) instead of a 302. TripTrace's own 401s are
...
export const oauthApi = {
  /** Validate OAuth authorize params â€?called by consent page on load */
  validate: (params: {
...
export const mapsApi = {
  ...
  route: (waypoints: { lat: number; lng: number }[], profile: 'driving' | 'walking' | 'transit' = 'driving', signal?: AbortSignal) =>
      apiClient.post('/maps/route', { waypoints, profile }, { signal }).then(r => r.data),
  routeSegments: (waypoints: { lat: number; lng: number }[], signal?: AbortSignal) =>
      apiClient.post('/maps/route/segments', { waypoints }, { signal }).then(r => r.data),
}
