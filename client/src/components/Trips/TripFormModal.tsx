...
interface TripFormModalProps {
  isOpen: boolean
  onClose: () => void
  onSave: (data: Record<string, string | number | null>) => Promise<{ trip?: { id?: number } } | void> | { trip?: { id?: number } } | void
  trip: Trip | null
...
export default function TripFormModal({ isOpen, onClose, onSave, trip, onCoverUpdate }: TripFormModalProps) {
  ...
  const showFullFields = isEditing
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    setError('')
    if (!formData.title.trim()) { setError('请填写旅行标题。'); return }
    if (formData.title.trim().length > 120) { setError('旅行标题不能超过 120 个字。'); return }
    if (formData.start_date && formData.end_date && new Date(formData.end_date) < new Date(formData.start_date)) {
      setError('结束日期不能早于开始日期。'); return
    }
    if (!formData.start_date && !formData.end_date) {
      const days = Number(formData.day_count)
      if (!Number.isFinite(days) || days < 1 || days > 365) {
        setError('未设置日期时，天数必须填写 1 到 365 之间的数字。'); return
      }
    }
    setIsLoading(true)
    try {
      const result = await onSave(isEditing ? {
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4" onPaste={handlePaste}>
        ...
        {showFullFields && canUploadCover && <div>
        ...
        {showFullFields && <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">
        ...
        {showFullFields && !formData.start_date && !formData.end_date && (
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">
        ...
        {showFullFields && (!isEditing || trip?.user_id === currentUser?.id || currentUser?.role === 'admin') && (
        ...
        {/* Members */}
        {showFullFields && allUsers.filter(u => u.id !== currentUser?.id).length > 0 && (
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">
