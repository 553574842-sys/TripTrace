...
const MAP_PRESETS: MapPreset[] = [
  { name: '高德标准地图', url: 'https://webrd01.is.autonavi.com/appmaptile?lang=zh_cn&size=1&scale=1&style=7&x={x}&y={y}&z={z}' },
  { name: '高德浅色地图', url: 'https://webrd01.is.autonavi.com/appmaptile?lang=zh_cn&size=1&scale=1&style=8&x={x}&y={y}&z={z}' },
]
...
export default function MapSettingsTab(): React.ReactElement {
  ...
  const [mapTileUrl, setMapTileUrl] = useState<string>(settings.map_tile_url || MAP_PRESETS[0].url)
  ...
  const [defaultLat, setDefaultLat] = useState<number | string>(settings.default_lat || 39.9042)
  const [defaultLng, setDefaultLng] = useState<number | string>(settings.default_lng || 116.4074)
  const [defaultZoom, setDefaultZoom] = useState<number | string>(settings.default_zoom || 11)

  useEffect(() => {
    setProvider((settings.map_provider as Provider) || 'leaflet')
    setMapTileUrl(settings.map_tile_url || MAP_PRESETS[0].url)
    ...
    setDefaultLat(settings.default_lat || 39.9042)
    setDefaultLng(settings.default_lng || 116.4074)
    setDefaultZoom(settings.default_zoom || 11)
  }, [settings])
      ...
      {provider === 'leaflet' && (
        <div>
          ...
          <input
            ...
            placeholder="https://webrd01.is.autonavi.com/appmaptile?lang=zh_cn&size=1&scale=1&style=7&x={x}&y={y}&z={z}"
        ...
        <div style={{ position: 'relative', inset: 0, height: '200px', width: '100%' }}>
          {provider === 'mapbox-gl' ? (
            <MapboxPreview
              token={mapboxToken}
              style={mapboxStyle}
              lat={parseFloat(String(defaultLat)) || 39.9042}
              lng={parseFloat(String(defaultLng)) || 116.4074}
