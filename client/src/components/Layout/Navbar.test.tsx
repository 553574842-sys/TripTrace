...
describe('Navbar', () => {
  ...
  it('FE-COMP-NAVBAR-002: shows TripTrace logo/brand', () => {
    render(<Navbar />);
    // The Navbar shows the app icon â€?check for presence of the nav element
  ...
  it('FE-COMP-NAVBAR-014: notification bell is rendered when user is logged in', () => {
    render(<Navbar />);
    // InAppNotificationBell is rendered â€?check that body has some content
  ...
  it('FE-COMP-NAVBAR-026: notification bell visible when tripId provided', () => {
    ...
    // InAppNotificationBell renders a button â€?check it is present
    const buttons = screen.getAllByRole('button');
    expect(buttons.length).toBeGreaterThan(0);
  });
  ...
  it('FE-COMP-NAVBAR-030: dark mode auto uses system preference', () => {
    // 'auto' dark_mode relies on matchMedia â€?seed with auto and render
