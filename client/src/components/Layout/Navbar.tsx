...
import { Plane, LogOut, Settings, ChevronDown, Shield, ArrowLeft, Users, Moon, Sun, Bell, Briefcase } from 'lucide-react'
    ...
    <nav style={{
      ...
      <div className="flex items-center gap-3 min-w-0">
        ...
        <Link to="/dashboard" className="flex items-center transition-colors flex-shrink-0">
          <img src={dark ? '/icons/icon-white.svg' : '/icons/icon-dark.svg'} alt="TripTrace" className="sm:hidden" style={{ height: 22, width: 22 }} />
          <img src={dark ? '/logo-light.svg' : '/logo-dark.svg'} alt="TripTrace" className="hidden sm:block" style={{ height: 28 }} />
        </Link>

        {/* Primary product navigation. Advanced addons stay available from settings/admin. */}
        {!tripTitle && (
          <>
            ...
            <Link to="/notifications"
              className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium transition-colors flex-shrink-0"
              style={{
                color: location.pathname === '/notifications' ? 'var(--text-primary)' : 'var(--text-muted)',
                background: location.pathname === '/notifications' ? 'var(--bg-hover)' : 'transparent',
              }}
              onMouseEnter={e => e.currentTarget.style.background = 'var(--bg-hover)'}
              onMouseLeave={e => { if (location.pathname !== '/notifications') e.currentTarget.style.background = 'transparent' }}>
              <Bell className="w-3.5 h-3.5" />
              <span className="hidden md:inline">{t('notifications.title')}</span>
            </Link>
            <Link to="/settings"
              className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-medium transition-colors flex-shrink-0"
              style={{
                color: location.pathname === '/settings' ? 'var(--text-primary)' : 'var(--text-muted)',
                background: location.pathname === '/settings' ? 'var(--bg-hover)' : 'transparent',
              }}
              onMouseEnter={e => e.currentTarget.style.background = 'var(--bg-hover)'}
                    ...
                    <div className="px-4 pt-2 pb-2.5 text-center" style={{ marginTop: 4, borderTop: '1px solid var(--border-secondary)' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
                        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: 'var(--bg-tertiary)', borderRadius: 99, padding: '4px 12px' }}>
                          <img src={dark ? '/text-light.svg' : '/text-dark.svg'} alt="TripTrace" style={{ height: 10, opacity: 0.5 }} />
