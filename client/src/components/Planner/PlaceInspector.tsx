    ...
    >
      <div style={{
        ...
        <div style={{ padding: '10px 16px', borderTop: '1px solid var(--border-faint)', display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          {place.lat && place.lng && (
            <ActionButton onClick={() => window.open(`https://uri.amap.com/marker?position=${place.lng},${place.lat}&name=${encodeURIComponent(place.name || '')}`, '_blank')} variant="ghost" icon={<Navigation size={13} />}
