...
import { calculateRoute, generateAmapUrl, optimizeRoute } from '../Map/RouteCalculator'
...
const DayPlanSidebar = React.memo(function DayPlanSidebar({
  ...
  const [deleteConfirmDay, setDeleteConfirmDay] = useState<Day | null>(null)
  ...
  const requestDeleteDay = (day: Day, e: React.MouseEvent<HTMLButtonElement>) => {
    e.stopPropagation()
    setDeleteConfirmDay(day)
  }

  const confirmDeleteDay = async () => {
    if (!deleteConfirmDay || isDeletingDay) return
    const dayId = deleteConfirmDay.id
    setIsDeletingDay(true)
    try {
      await tripActions.deleteDay(tripId, dayId)
      if (selectedDayId === dayId) {
        const nextDay = days.find(d => d.id !== dayId)
        onSelectDay(nextDay?.id ?? null)
      }
      setDeleteConfirmDay(null)
      toast.success(t('dayplan.toast.dayDeleted'))
    } catch (err: unknown) {
      toast.error(err instanceof Error ? err.message : t('dayplan.toast.deleteDayError'))
    } finally {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', position: 'relative', fontFamily: "-apple-system, BlinkMacSystemFont, 'SF Pro Text', system-ui, sans-serif" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  {editingDayId === day.id ? (
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 5, minWidth: 0 }}>
                      ...
                      {canEditDays && (
                        <Tooltip label={t('dayplan.deleteDay')} placement="top">
                        <button
                          onClick={e => requestDeleteDay(day, e)}
                        ...
                        return (
                          ...
                          <div
                            ...
                            onContextMenu={e => ctxMenu.open(e, [
                              ...
                              (place.lat && place.lng) && { label: '高德地图', icon: Navigation, onClick: () => window.open(`https://uri.amap.com/marker?position=${place.lng},${place.lat}&name=${encodeURIComponent(place.name || '')}`, '_blank') },
                              { divider: true },
      ...
      {/* Confirm: delete day */}
      {deleteConfirmDay && ReactDOM.createPortal(
        <div style={{
          position: 'fixed', inset: 0, zIndex: 1000,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          background: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(3px)',
        }} onClick={() => !isDeletingDay && setDeleteConfirmDay(null)}>
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-primary)' }}>
                {t('dayplan.deleteDayTitle')}
              </div>
            </div>
            <div style={{ fontSize: 12.5, color: 'var(--text-secondary)', lineHeight: 1.5 }}>
              {t('dayplan.deleteDayBody', { title: deleteConfirmDay.title || t('dayplan.dayN', { n: days.findIndex(d => d.id === deleteConfirmDay.id) + 1 }) })}
            </div>
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 4 }}>
              <button
                ...
                style={{
                  ...
                  borderRadius: 8, padding: '6px 14px', cursor: isDeletingDay ? 'default' : 'pointer', color: 'var(--text-muted)', fontFamily: 'inherit',
                  opacity: isDeletingDay ? 0.5 : 1,
                }}
              ...
              <button
                ...
                style={{
                  ...
                  border: 'none', borderRadius: 8, padding: '6px 16px', cursor: isDeletingDay ? 'default' : 'pointer', fontWeight: 600, fontFamily: 'inherit',
                  opacity: isDeletingDay ? 0.7 : 1,
                }}
              >{isDeletingDay ? t('common.loading') : t('common.delete')}</button>
            </div>
          </div>
        </div>,
        document.body
