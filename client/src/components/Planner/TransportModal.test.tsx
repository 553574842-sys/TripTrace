...
function getAttachmentFileInput() {
  const inputs = Array.from(document.querySelectorAll('input[type="file"]')) as HTMLInputElement[];
  return inputs.find(input => input.accept.includes('.pdf'))!;
}
...
describe('TransportModal', () => {
  ...
  it('FE-PLANNER-TRANSMODAL-002: shows Chinese create title for new transport', () => {
    render(<TransportModal {...defaultProps} reservation={null} />);
    expect(screen.getByText('添加交通')).toBeInTheDocument();
  });

  it('FE-PLANNER-TRANSMODAL-003: shows Chinese edit title when editing', () => {
    const res = buildReservation({ title: 'Paris Flight', type: 'flight' });
    render(<TransportModal {...defaultProps} reservation={res} />);
    expect(screen.getByText('编辑交通')).toBeInTheDocument();
  });
  ...
  it('FE-PLANNER-TRANSMODAL-005: all 4 Chinese transport type buttons are visible', () => {
    render(<TransportModal {...defaultProps} />);
    expect(screen.getByRole('button', { name: /^航班$/ })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /^火车$/ })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /^汽车$/ })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /^邮轮$/ })).toBeInTheDocument();
  });
  ...
  it('FE-PLANNER-TRANSMODAL-007: edit mode save button shows Chinese update label', () => {
    const res = buildReservation({ title: 'My Train', type: 'train' });
    render(<TransportModal {...defaultProps} reservation={res} />);
    expect(screen.getByRole('button', { name: /^更新$/ })).toBeInTheDocument();
  });

  it('FE-PLANNER-TRANSMODAL-008: Chinese cancel button calls onClose', async () => {
    const onClose = vi.fn();
  ...
  it('FE-PLANNER-TRANSMODAL-009: submitting valid flight calls onSave with correct type', async () => {
    const onSave = vi.fn().mockResolvedValue(undefined);
    render(<TransportModal {...defaultProps} onSave={onSave} />);
    await userEvent.type(screen.getByPlaceholderText(/如：全季酒店/i), 'LH456');
    await userEvent.click(screen.getByRole('button', { name: /^添加$/ }));
  ...
  it('FE-PLANNER-TRANSMODAL-010: switching to train type calls onSave with train type', async () => {
    const onSave = vi.fn().mockResolvedValue(undefined);
    render(<TransportModal {...defaultProps} onSave={onSave} />);
    await userEvent.click(screen.getByRole('button', { name: /^火车$/ }));
    await userEvent.type(screen.getByPlaceholderText(/如：全季酒店/i), 'G1234');
    await userEvent.click(screen.getByRole('button', { name: /^添加$/ }));
    await waitFor(() => expect(onSave).toHaveBeenCalled());
  ...
  it('FE-PLANNER-TRANSMODAL-025: pending files flushed after saving new transport', async () => {
    ...
    await userEvent.type(screen.getByPlaceholderText(/如：全季酒店/i), 'LH001');
    await userEvent.click(screen.getByRole('button', { name: /^添加$/ }));
  ...
  it.each([
    ['AI_KEY_INVALID', 'AI API Key 无效或没有权限，请检查密钥是否填错。'],
    ['AI_MODEL_UNAVAILABLE', '当前 AI 模型不可用，请检查 AI_MODEL 配置或更换模型。'],
    ['AI_IMAGE_TOO_LARGE', '图片太大，请压缩截图或改用文字描述。'],
    ['AI_RESPONSE_PARSE_FAILED', 'AI 返回格式无法解析，请换一张更清晰的截图或改用文字描述。'],
  ])('FE-PLANNER-TRANSMODAL-AI: displays a clear message for %s', async (code, expectedMessage) => {
    server.use(
      http.post('/api/ai/transport/parse', () =>
        HttpResponse.json({ code, error: 'Request failed with status code 500' }, { status: 500 }),
      ),
    );
    ...
    render(<TransportModal {...defaultProps} />);
    fireEvent.change(screen.getByPlaceholderText(/南航 CZ3123/), {
      target: { value: '测试交通订单' },
    });
    await userEvent.click(screen.getByRole('button', { name: /^AI 识别$/ }));

    await waitFor(() => {
      expect(addToast).toHaveBeenCalledWith(expectedMessage, 'error', undefined);
    });
    expect(addToast).not.toHaveBeenCalledWith(expect.stringMatching(/status code 500/i), 'error', undefined);
    delete window.__addToast;
  });

  it('FE-PLANNER-TRANSMODAL-AI: hides unknown provider errors behind a Chinese fallback', async () => {
    server.use(
      http.post('/api/ai/transport/parse', () =>
    ...
    await waitFor(() => {
      expect(addToast).toHaveBeenCalledWith('AI 识别失败', 'error', undefined);
    });
    delete window.__addToast;
  });
});
