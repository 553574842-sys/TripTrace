// FE-PLANNER-RESMODAL-001 至 FE-PLANNER-RESMODAL-054
...
describe('ReservationModal', () => {
  ...
  it('FE-PLANNER-RESMODAL-002: 新建时显示“添加预订信息”', () => {
    render(<ReservationModal {...defaultProps} reservation={null} />);
    expect(screen.getByText('添加预订信息')).toBeInTheDocument();
  });

  it('FE-PLANNER-RESMODAL-003: 编辑时显示“编辑预订信息”', () => {
    const res = buildReservation({ title: '晚餐订位', type: 'restaurant' });
    render(<ReservationModal {...defaultProps} reservation={res} />);
    expect(screen.getByText('编辑预订信息')).toBeInTheDocument();
  });
  ...
  it('FE-PLANNER-RESMODAL-005: all 5 type buttons are visible (transport types removed)', () => {
    ...
    expect(screen.getByRole('button', { name: /餐厅订位/ })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /门票 \/ 活动/ })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /跟团 \/ 一日游/ })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /其他预订/ })).toBeInTheDocument();
    expect(screen.queryByRole('button', { name: /^航班$/ })).not.toBeInTheDocument();
    expect(screen.queryByRole('button', { name: /^火车$/ })).not.toBeInTheDocument();
    expect(screen.queryByRole('button', { name: /^汽车$/ })).not.toBeInTheDocument();
    expect(screen.queryByRole('button', { name: /^邮轮$/ })).not.toBeInTheDocument();
  });
  ...
  it('FE-PLANNER-RESMODAL-006: clicking Event type button activates it', async () => {
    render(<ReservationModal {...defaultProps} />);
    const eventBtn = screen.getByRole('button', { name: /门票 \/ 活动/ });
  ...
  it('FE-PLANNER-RESMODAL-008: hotel type shows check-in/check-out time fields', async () => {
    render(<ReservationModal {...defaultProps} />);
    await userEvent.click(screen.getByRole('button', { name: /住宿/ }));
    const checkInLabels = screen.getAllByText(/入住/);
    expect(checkInLabels.length).toBeGreaterThanOrEqual(1);
    expect(screen.getByText('退房时间')).toBeInTheDocument();
  });

  it('FE-PLANNER-RESMODAL-009: restaurant type shows location field', async () => {
    render(<ReservationModal {...defaultProps} />);
    await userEvent.click(screen.getByRole('button', { name: /餐厅订位/ }));
    expect(screen.getByPlaceholderText(/输入地点名称/)).toBeInTheDocument();
  });

  it('FE-PLANNER-RESMODAL-010: hotel type hides assignment picker', async () => {
    const day = buildDay({ id: 1, title: '抵达北京' });
    const place = buildPlace({ name: '故宫博物院' });
    ...
    await userEvent.click(screen.getByRole('button', { name: /住宿/ }));
    expect(screen.queryByText(/关联行程安排/)).not.toBeInTheDocument();
  });

  // ── Form population from existing reservation ──────────────────────────────

  it('FE-PLANNER-RESMODAL-011: editing pre-fills title', () => {
    const res = buildReservation({ title: '全季酒店', type: 'hotel', status: 'confirmed' });
    render(<ReservationModal {...defaultProps} reservation={res} />);
    expect(screen.getByDisplayValue('全季酒店')).toBeInTheDocument();
  });
  ...
  it('FE-PLANNER-RESMODAL-013: editing pre-fills notes', () => {
    const res = buildReservation({ notes: '包含早餐' });
    render(<ReservationModal {...defaultProps} reservation={res} />);
    expect(screen.getByDisplayValue('包含早餐')).toBeInTheDocument();
  });

  it('FE-PLANNER-RESMODAL-014: editing pre-fills type — restaurant type shows location field', () => {
    const res = buildReservation({ type: 'restaurant', location: '北京市东城区东华门大街' });
    render(<ReservationModal {...defaultProps} reservation={res} />);
    expect(screen.getByDisplayValue('北京市东城区东华门大街')).toBeInTheDocument();
  });
  ...
  it('FE-PLANNER-RESMODAL-015: end datetime before start shows error and blocks submit', async () => {
    ...
    // Fill in the title
    await userEvent.type(screen.getByPlaceholderText(/如：全季酒店/), '故宫门票');
    ...
    expect(addToast).toHaveBeenCalledWith(
      expect.stringMatching(/结束日期\/时间必须晚于开始/),
      'error',
      undefined,
    );
  ...
  it('FE-PLANNER-RESMODAL-016: submitting valid restaurant booking calls onSave with correct shape', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: /餐厅订位/ }));
    await userEvent.type(screen.getByPlaceholderText(/如：全季酒店/), '四季民福烤鸭店');

    await userEvent.click(screen.getByRole('button', { name: /^添加$/ }));

    await waitFor(() => expect(onSave).toHaveBeenCalled());
    expect(onSave).toHaveBeenCalledWith(
      expect.objectContaining({ title: '四季民福烤鸭店', type: 'restaurant' })
    );
  });
  ...
  it('FE-PLANNER-RESMODAL-017: status confirmed — onSave called with status confirmed', async () => {
    ...
    await userEvent.type(screen.getByPlaceholderText(/如：全季酒店/), '测试预订');
    ...
    const res = buildReservation({ status: 'confirmed', type: 'event', title: '故宫门票' });
    const { unmount } = render(<ReservationModal {...defaultProps} reservation={res} onSave={onSave} />);
    const updateBtn = screen.getAllByRole('button', { name: /更新/ })[0];
  ...
  it('FE-PLANNER-RESMODAL-020: assignment picker appears when days/assignments are populated (non-hotel)', () => {
    ...
    expect(screen.getByText('关联行程安排')).toBeInTheDocument();
  });
  ...
  it('FE-PLANNER-RESMODAL-024: budget section visible when budget addon is enabled', () => {
    ...
    render(<ReservationModal {...defaultProps} />);
    expect(screen.getByText('预订金额')).toBeInTheDocument();
    expect(screen.getByText(/费用分类/)).toBeInTheDocument();
  });
  ...
  it('FE-PLANNER-RESMODAL-026: budget hint shown when price > 0', async () => {
    ...
    expect(screen.getByText(/保存后会自动计入行程花费/)).toBeInTheDocument();
  });
  ...
  it('FE-PLANNER-RESMODAL-027: budget fields included in onSave when price is set', async () => {
    ...
    await userEvent.type(screen.getByPlaceholderText(/如：全季酒店/), '北京饭店');
  ...
  it('FE-PLANNER-RESMODAL-030: hotel type — saving calls onSave with correct hotel shape', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: /住宿/ }));
    await userEvent.type(screen.getByPlaceholderText(/如：全季酒店/), '全季酒店');
    await userEvent.click(screen.getByRole('button', { name: /^添加$/ }));

    await waitFor(() => expect(onSave).toHaveBeenCalled());
    expect(onSave).toHaveBeenCalledWith(
      expect.objectContaining({ title: '全季酒店', type: 'hotel' })
    );
  });
  ...
  it('FE-PLANNER-RESMODAL-031: event type — saving calls onSave with event type', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: /门票 \/ 活动/ }));
    await userEvent.type(screen.getByPlaceholderText(/如：全季酒店/), '故宫门票');
    await userEvent.click(screen.getByRole('button', { name: /^添加$/ }));

    await waitFor(() => expect(onSave).toHaveBeenCalled());
    expect(onSave).toHaveBeenCalledWith(
      expect.objectContaining({ title: '故宫门票', type: 'event' })
    );
  });

  it('FE-PLANNER-RESMODAL-032: edit mode — save button shows "Update"', () => {
    const res = buildReservation({ title: '长城讲解服务', type: 'other' });
    render(<ReservationModal {...defaultProps} reservation={res} />);
  ...
  it('FE-PLANNER-RESMODAL-033: modal is closed when isOpen=false', () => {
    ...
    expect(screen.queryByText(/添加预订信息/)).not.toBeInTheDocument();
  });

  it('FE-PLANNER-RESMODAL-034: location and confirmation number inputs are present', () => {
    render(<ReservationModal {...defaultProps} />);
    expect(screen.getByPlaceholderText(/输入地点名称/)).toBeInTheDocument();
    expect(screen.getByPlaceholderText(/输入平台订单号/)).toBeInTheDocument();
  });
  ...
  it('FE-PLANNER-RESMODAL-036: file upload to existing reservation calls onFileUpload', async () => {
    ...
    const res = buildReservation({ id: 10, title: '长城讲解服务', type: 'other' });
    render(
      <ReservationModal
        {...defaultProps}
        reservation={res}
  ...
  it('FE-PLANNER-RESMODAL-037: link existing file button appears when unattached files exist', () => {
    ...
    expect(screen.getByRole('button', { name: /关联已有附件/ })).toBeInTheDocument();
  });
  ...
  it('FE-PLANNER-RESMODAL-038: clicking "link existing file" shows file picker dropdown', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: /关联已有附件/ }));
  ...
  it('FE-PLANNER-RESMODAL-039: clicking file in picker links it and closes picker', async () => {
    ...
    await waitFor(() => {
      expect(screen.queryByRole('button', { name: /关联已有附件/ })).not.toBeInTheDocument();
    });
  });
  ...
  it('FE-PLANNER-RESMODAL-044: budget category dropdown options include existing categories', () => {
    ...
    seedStore(useTripStore, {
      trip: buildTrip({ id: 1 }),
      budgetItems: [
        { id: 1, trip_id: 1, name: '高铁票', amount: 300, currency: 'CNY', category: '交通', paid_by: null, persons: 1, members: [], expense_date: null },
      ],
    });
    render(<ReservationModal {...defaultProps} />);
  ...
  it('FE-PLANNER-RESMODAL-045: tour type shows time pickers', async () => {
    render(<ReservationModal {...defaultProps} />);
    await userEvent.click(screen.getByRole('button', { name: /^跟团 \/ 一日游$/ }));
    await waitFor(() => {
  ...
  it('FE-PLANNER-RESMODAL-046: other type renders and saves correctly', async () => {
    const onSave = vi.fn().mockResolvedValue(undefined);
    render(<ReservationModal {...defaultProps} onSave={onSave} />);
    await userEvent.click(screen.getByRole('button', { name: /^其他预订$/ }));
    await userEvent.type(screen.getByPlaceholderText(/如：全季酒店/), '其他预订');
    await userEvent.click(screen.getByRole('button', { name: /^添加$/ }));
  ...
  it('FE-PLANNER-RESMODAL-047: clicking budget category select changes the value', async () => {
    ...
    seedStore(useTripStore, {
      trip: buildTrip({ id: 1 }),
      budgetItems: [
        { id: 1, trip_id: 1, name: '门票', amount: 100, currency: 'CNY', category: '交通', paid_by: null, persons: 1, members: [], expense_date: null },
      ],
    });
    render(<ReservationModal {...defaultProps} />);

    // Open the budget category CustomSelect (shows placeholder "Auto (from booking type)")
    const budgetCategoryBtn = screen.getByText(/按预订类型自动分类/).closest('button')!;
    await userEvent.click(budgetCategoryBtn);

    // Click the "Transport" category option
    await waitFor(() => expect(screen.getByText('交通')).toBeInTheDocument());
    await userEvent.click(screen.getByText('交通'));
  ...
  it('FE-PLANNER-RESMODAL-035: hotel type saves correctly', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: /^住宿$/ }));
    await userEvent.type(screen.getByPlaceholderText(/如：全季酒店/), '住宿测试');
    await userEvent.click(screen.getByRole('button', { name: /^添加$/ }));
  ...
  it('FE-PLANNER-RESMODAL-050: non-monotonic IDs — end picker with low ID does not clobber start', async () => {
    ...
    // Switch to hotel type
    await userEvent.click(screen.getByRole('button', { name: /^住宿$/ }));
    await userEvent.type(screen.getByPlaceholderText(/如：全季酒店/), '跨日期酒店');

    // Open start picker (first "Select day" trigger) and select Day 1 (id=17)
    const startTrigger = () => screen.getAllByRole('button').filter(b => b.textContent?.includes('选择行程日期') || b.textContent?.startsWith('第 '))[0];
    await userEvent.click(startTrigger());
    await userEvent.click(screen.getAllByRole('button').find(b => b.textContent?.startsWith('第 1 天'))!);

    // Open end picker and select Day 16 (id=7, low ID but last positionally)
    const endTrigger = () => screen.getAllByRole('button').filter(b => b.textContent?.includes('选择行程日期') || /^第 \d+ 天/.test(b.textContent?.trim() ?? ''))[1];
    await userEvent.click(endTrigger());
    await userEvent.click(screen.getAllByRole('button').find(b => b.textContent?.startsWith('第 16 天'))!);
  ...
  it('FE-PLANNER-RESMODAL-051: non-monotonic IDs — start picker does not collapse end when start has high ID', async () => {
    ...
    await userEvent.click(screen.getByRole('button', { name: /^住宿$/ }));
    await userEvent.type(screen.getByPlaceholderText(/如：全季酒店/), '多日酒店');
    ...
    const startTrigger = () => screen.getAllByRole('button').filter(b => b.textContent?.includes('选择行程日期') || /^第 \d+ 天/.test(b.textContent?.trim() ?? ''))[0];
    await userEvent.click(startTrigger());
    await userEvent.click(screen.getAllByRole('button').find(b => b.textContent?.startsWith('第 9 天'))!);
  ...
  it('FE-PLANNER-RESMODAL-052: hotel with no accommodation_id sends assignment_id as null (issue #934)', async () => {
    ...
    const res = buildReservation({
      id: 10, title: '旧住宿记录', type: 'hotel', status: 'confirmed',
  ...
  it('FE-PLANNER-RESMODAL-053: 附件为空时显示清晰的中文引导', () => {
    render(<ReservationModal {...defaultProps} />);

    expect(screen.getByText('附件')).toBeInTheDocument();
    expect(screen.getByText('暂无附件，可添加订单截图、电子票或确认单')).toBeInTheDocument();
  });

  it('FE-PLANNER-RESMODAL-054: 住宿预订使用地道的入住和退房文案', async () => {
    render(<ReservationModal {...defaultProps} />);

    await userEvent.click(screen.getByRole('button', { name: /^住宿$/ }));

    expect(screen.getByText('住宿地点')).toBeInTheDocument();
    expect(screen.getByText('入住日期')).toBeInTheDocument();
    expect(screen.getByText('退房日期')).toBeInTheDocument();
    expect(screen.getByText('最早入住时间')).toBeInTheDocument();
    expect(screen.getByText('最晚入住时间')).toBeInTheDocument();
    expect(screen.getByText('退房时间')).toBeInTheDocument();
  });
});
