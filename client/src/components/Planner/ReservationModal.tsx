    ...
    try {
      ...
      if (isBudgetEnabled) {
        saveData.create_budget_entry = form.price && parseFloat(form.price) > 0
          ? { total_price: parseFloat(form.price), category: form.budget_category || t(`reservations.type.${form.type}`) || t('reservations.type.other') }
          : { total_price: 0 }
      }
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div>
                ...
                <CustomSelect
                  ...
                  options={[
                    { value: '', label: t('reservations.meta.noHotelPlace') },
        ...
        {/* Files */}
        <div>
          <label style={labelStyle}>{t('reservations.attachments')}</label>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            {attachedFiles.length === 0 && pendingFiles.length === 0 && (
              <div style={{ padding: '8px 10px', fontSize: 11, color: 'var(--text-faint)', background: 'var(--bg-secondary)', borderRadius: 8 }}>
                {t('reservations.attachmentsEmpty')}
              </div>
            )}
            {attachedFiles.map(f => (
