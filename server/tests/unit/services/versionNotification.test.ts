...
vi.mock('../../../src/config', () => ({
  JWT_SECRET: 'test-jwt-secret-for-triptrace-testing-only',
...
function mockGitHubLatest(tagName: string, ok = true): void {
  vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
    ok,
    json: async () => ({ tag_name: tagName, html_url: `https://example.invalid/releases/tag/${tagName}` }),
  }));
}
...
beforeEach(() => {
  process.env.PROJECT_REPOSITORY = 'triptrace/triptrace';
  resetTestDb(testDb);
...
afterAll(() => {
  delete process.env.PROJECT_REPOSITORY;
  testDb.close();
  vi.unstubAllGlobals();
});
