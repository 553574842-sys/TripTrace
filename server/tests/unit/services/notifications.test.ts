...
describe('buildWebhookBody', () => {
  ...
  it('Discord embed footer defaults to TripTrace when no trip name', () => {
    ...
    expect(body.embeds[0].footer.text).toBe('TripTrace');
  });
  ...
  it('generic URL produces plain JSON with original fields plus timestamp and source', () => {
    ...
    expect(body).toHaveProperty('source', 'TripTrace');
  });
});
...
describe('buildEmailHtml', () => {
  ...
  it('uses English i18n strings for lang=en', () => {
    ...
    expect(html).toContain('notifications enabled in TripTrace');
  });

  it('uses German i18n strings for lang=de', () => {
    const html = buildEmailHtml('Subject', 'Body', 'de');
    expect(html).toContain('TripTrace aktiviert');
  });
  ...
  it('falls back to English i18n for unknown language', () => {
    ...
    expect(unknown).toContain('notifications enabled in TripTrace');
  });
});
...
describe('sendNtfy', () => {
  ...
  it('NTFY-004 -includes Click header when link is provided', async () => {
    ...
    await sendNtfy(ntfyUrl, null, { ...payload, link: 'https://triptrace.example.com/trips/5' });

    const [, calledOpts] = mockFetch.mock.calls[0];
    expect(calledOpts.headers['Click']).toBe('https://triptrace.example.com/trips/5');
  });
  ...
  it('NTFY-009 - title with non-Latin-1 chars is RFC 2047 encoded', async () => {
    ...
    await sendNtfy(ntfyUrl, null, { ...payload, title: '买票提醒' });

    const [, calledOpts] = mockFetch.mock.calls[0];
    const encoded = calledOpts.headers['Title'] as string;
    expect(encoded).toMatch(/^=\?UTF-8\?B\?/);
    const b64 = encoded.replace(/^=\?UTF-8\?B\?/, '').replace(/\?=$/, '');
    expect(Buffer.from(b64, 'base64').toString('utf8')).toBe('买票提醒');
  });
