...
const { testDb, dbMock, uploadsRoot } = vi.hoisted(() => {
  const path = require('path');
  const os = require('os');
  const root = path.join(os.tmpdir(), 'triptrace-test-uploads-files');
  process.env.TRIPTRACE_UPLOAD_ROOT = root;
  ...
  return { testDb: db, dbMock: mock, uploadsRoot: root };
});
...
const uploadsDir = path.join(uploadsRoot, 'files');
