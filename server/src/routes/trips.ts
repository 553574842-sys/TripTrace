...
import { ensureWritableDir, getUploadDir } from '../services/uploadPaths';
...
const coverStorage = multer.diskStorage({
  destination: (_req, _file, cb) => {
    try { cb(null, ensureWritableDir(coversDir)); } catch (err) { cb(err as Error, coversDir); }
  },
...
router.post('/', authenticate, (req: Request, res: Response) => {
  ...
  if (!checkPermission('trip_create', authReq.user.role, null, authReq.user.id, false))
    return res.status(403).json({ error: '你没有创建旅行的权限。' });

  const { title, description, currency, reminder_days, day_count } = req.body;
  const cleanTitle = typeof title === 'string' ? title.trim() : '';
  if (!cleanTitle) return res.status(400).json({ error: '请填写旅行标题。' });
  if (cleanTitle.length > 120) return res.status(400).json({ error: '旅行标题不能超过 120 个字。' });
  ...
  if (start_date && !isValidDate(start_date)) return res.status(400).json({ error: '开始日期格式不正确，请重新选择。' });
  if (end_date && !isValidDate(end_date)) return res.status(400).json({ error: '结束日期格式不正确，请重新选择。' });
  ...
  const rawDayCount = day_count !== undefined ? Number(day_count) : undefined;
  if (!start_date && !end_date && rawDayCount !== undefined && (!Number.isFinite(rawDayCount) || rawDayCount < 1 || rawDayCount > 365)) {
    return res.status(400).json({ error: '未设置日期时，天数必须填写 1 到 365 之间的数字。' });
  }
  const parsedDayCount = rawDayCount !== undefined ? Math.min(Math.max(rawDayCount || 7, 1), 365) : undefined;

  try {
    const { trip, tripId, reminderDays } = createTrip(authReq.user.id, { title: cleanTitle, description, start_date, end_date, currency, reminder_days, day_count: parsedDayCount });

    writeAudit({ userId: authReq.user.id, action: 'trip.create', ip: getClientIp(req), details: { tripId, title: cleanTitle, reminder_days: reminderDays === 0 ? 'none' : `${reminderDays} days` } });
    if (reminderDays > 0) {
      logInfo(`${authReq.user.email} set ${reminderDays}-day reminder for trip "${cleanTitle}"`);
    }

    return res.status(201).json({ trip });
  } catch (e: any) {
    if (e instanceof ValidationError) return res.status(400).json({ error: e.message });
    console.error('[Trips] Failed to create trip:', e);
    return res.status(500).json({ error: '服务器创建旅行失败，请检查填写内容后再试。' });
  }
});
...
router.get('/:id/bundle', authenticate, (req: Request, res: Response) => {
  ...
  const allMembers = [owner, ...(members || [])].filter((member): member is NonNullable<typeof member> => Boolean(member));
