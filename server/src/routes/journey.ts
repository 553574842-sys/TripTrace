...
import { ensureWritableDir, getUploadDir, getUploadsRoot } from '../services/uploadPaths';

const router = express.Router();

const uploadsBase = getUploadDir('journey');

const storage = multer.diskStorage({
  destination: (_req, _file, cb) => {
    try { cb(null, ensureWritableDir(uploadsBase)); } catch (err) { cb(err as Error, uploadsBase); }
  },
...
router.delete('/photos/:photoId', authenticate, async (req: Request, res: Response) => {
  ...
  if (photo.file_path) {
    const fullPath = path.join(getUploadsRoot(), photo.file_path);
