...
oauthPublicRouter.post('/oauth/token', tokenLimiter, (req: Request, res: Response) => {
  ...
  // M1: RFC 6749 Section 5.1 -token responses must not be cached
...
// OIDC UserInfo endpoint (RFC 9068 / OpenID Connect Core Section 5.3)
...
oauthPublicRouter.get('/oauth/userinfo', (req: Request, res: Response) => {
  ...
  if (!auth || !auth.toLowerCase().startsWith('bearer ')) {
    res.set('WWW-Authenticate', 'Bearer realm="TripTrace MCP"');
    return res.status(401).json({ error: 'invalid_token' });
  }
  const token = auth.slice(7);
  const info = getUserByAccessToken(token);
  if (!info) {
    res.set('WWW-Authenticate', 'Bearer realm="TripTrace MCP", error="invalid_token"');
    return res.status(401).json({ error: 'invalid_token' });
  }
  return res.json({
...
oauthPublicRouter.post('/oauth/revoke', revokeLimiter, (req: Request, res: Response) => {
  ...
  // RFC 7009 Section 2.2: always respond 200 even if token was already revoked or not found
