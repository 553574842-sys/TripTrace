...
import { calculateAmapRoute, calculateAmapSegments, type RouteProfile } from '../services/amapRouteService';
import { db } from '../db/database';
import { serveFilePath } from '../services/placePhotoCache';

const router = express.Router();

function readRouteWaypoints(body: unknown): { lat: number; lng: number }[] {
  const waypoints = (body as { waypoints?: unknown })?.waypoints;
  if (!Array.isArray(waypoints)) return [];
  return waypoints.map((point) => ({
    lat: Number((point as { lat?: unknown }).lat),
    lng: Number((point as { lng?: unknown }).lng),
  }));
}

function readRouteProfile(body: unknown): RouteProfile {
  const profile = (body as { profile?: unknown })?.profile;
  if (profile === 'walking' || profile === 'transit') return profile;
  return 'driving';
}
...
// POST /route - domestic build: route calculation is served by Amap only.
router.post('/route', authenticate, async (req: Request, res: Response) => {
  const authReq = req as AuthRequest;

  try {
    const result = await calculateAmapRoute(authReq.user.id, readRouteWaypoints(req.body), readRouteProfile(req.body));
    res.json(result);
  } catch (err: unknown) {
    const status = (err as { status?: number }).status || 500;
    const message = err instanceof Error ? err.message : 'Route calculation error';
    console.error('Amap route error:', err);
    res.status(status).json({ error: message });
  }
});

// POST /route/segments - returns per-leg walking and driving estimates from Amap.
router.post('/route/segments', authenticate, async (req: Request, res: Response) => {
  const authReq = req as AuthRequest;

  try {
    const segments = await calculateAmapSegments(authReq.user.id, readRouteWaypoints(req.body));
    res.json({ segments });
  } catch (err: unknown) {
    const status = (err as { status?: number }).status || 500;
    const message = err instanceof Error ? err.message : 'Route segment calculation error';
    console.error('Amap route segments error:', err);
    res.status(status).json({ error: message });
  }
});
