...
function seedAdminAccount(db: Database.Database): void {
  try {
    ...
    if (isOidcOnlyConfigured()) {
      console.log('');
      console.log('=== TripTrace OIDC-Only Mode ===');
    ...
    console.log('');
    console.log('=== TripTrace First Run: Admin Account Created ===');
...
function seedAddons(db: Database.Database): void {
  try {
    const defaultAddons = [
      ...
      { id: 'vacay', name: 'Vacay', description: 'Legacy vacation planner (hidden in the domestic edition)', type: 'global', icon: 'CalendarDays', enabled: 0, sort_order: 10 },
      { id: 'atlas', name: 'Atlas', description: 'Legacy world map (hidden in the domestic edition)', type: 'global', icon: 'Globe', enabled: 0, sort_order: 11 },
