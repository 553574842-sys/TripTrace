...
import { ensureWritableDir, getDataRoot } from './uploadPaths';
...
function getProjectRepository(): string | null {
  const repo = process.env.PROJECT_REPOSITORY?.trim();
...
export async function getGithubReleases(perPage: string = '10', page: string = '1') {
  ...
  try {
    const resp = await fetch(
      ...
      { headers: { 'Accept': 'application/vnd.github.v3+json', 'User-Agent': 'TripTrace-Server' } }
    );
    if (!resp.ok) return [];
