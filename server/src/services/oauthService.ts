...
export function validateAuthorizeRequest(
  ...
  // H1: Enforce code_challenge format (RFC 7636 Section 4.2)
  ...
  // RFC 8707 resource indicator: if provided, must identify the TripTrace
  ...
  if (resource !== mcpResource) {
    return { valid: false, error: 'invalid_target', error_description: 'Requested resource must be the TripTrace MCP endpoint' };
  }
  ...
  // than rejecting the whole request (per OAuth 2.0 Section 3.3 scope narrowing).
