...
export function deleteNote(tripId: string | number, noteId: string | number): boolean {
  ...
  for (const f of noteFiles) {
    const filePath = path.join(getUploadsRoot(), f.filename);
...
export function deleteNoteFile(noteId: string | number, fileId: string | number): boolean {
  ...
  const filePath = path.join(getUploadsRoot(), file.filename);
  try { fs.unlinkSync(filePath); } catch { /* ignore */ }
  ...
  try {
    ...
    try {
      const r = await fetch(url, {
        ...
        headers: { 'User-Agent': 'Mozilla/5.0 (compatible; TripTrace/1.0)' },
      } as any);
