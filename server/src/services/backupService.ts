...
import { ensureWritableDir, getDataDir, getDataRoot, getUploadsRoot } from './uploadPaths';
    ...
    try {
      ...
      for (const table of requiredTables) {
        if (!tableNames.has(table)) {
          fs.rmSync(extractDir, { recursive: true, force: true });
          return { success: false, error: `Uploaded database is missing required table: ${table}. This does not appear to be a TripTrace backup.`, status: 400 };
        }
      }
    } catch (err) {
      fs.rmSync(extractDir, { recursive: true, force: true });
...
export function getUploadTmpDir(): string {
  return ensureWritableDir(getDataDir('tmp'));
}
