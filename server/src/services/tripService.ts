...
import { getUploadsRoot } from './uploadPaths';
...
export function getTrip(tripId: string | number, userId: number): (Trip & { owner_username?: string; day_count?: number; place_count?: number; is_owner?: number; shared_count?: number }) | undefined {
  return db.prepare(`
    ${TRIP_SELECT}
    LEFT JOIN trip_members m ON m.trip_id = t.id AND m.user_id = :userId
    WHERE t.id = :tripId AND (t.user_id = :userId OR m.user_id IS NOT NULL)
  `).get({ userId, tripId }) as (Trip & { owner_username?: string; day_count?: number; place_count?: number; is_owner?: number; shared_count?: number }) | undefined;
}
...
export function exportICS(tripId: string | number): { ics: string; filename: string } {
  ...
  let ics = 'BEGIN:VCALENDAR\r\nVERSION:2.0\r\nPRODID:-//TripTrace//Travel Planner//EN\r\nCALSCALE:GREGORIAN\r\nMETHOD:PUBLISH\r\n';
  ics += `X-WR-CALNAME:${esc(trip.title || 'TripTrace Trip')}\r\n`;
