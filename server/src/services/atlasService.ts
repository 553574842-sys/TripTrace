...
async function loadAdmin1Geo(): Promise<any> {
  ...
  admin1GeoLoading = fetch(
    ...
    { headers: { 'User-Agent': 'TripTrace Travel Planner' } }
  ).then(r => r.json() as Promise<FeatureCollection>).then(geo => {
    admin1GeoCache = geo;
...
// Shared throttle: enforces 鈮?.1s between any Nominatim request, across all callers.
