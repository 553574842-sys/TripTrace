...
import { ensureWritableDir, getUploadDir } from './uploadPaths';
...
const avatarDir = ensureWritableDir(getUploadDir('avatars'));
...
export function getAppSettings(userId: number): { error?: string; status?: number; data?: Record<string, string> } {
  ...
  for (const key of ADMIN_SETTINGS_KEYS) {
    ...
    if (row) result[key] = (key === 'smtp_pass' || key === 'admin_webhook_url' || key === 'admin_ntfy_token') ? ADMIN_SECRET_MASK : row.value;
  }
  return { data: result };
}
...
export function updateAppSettings(
  ...
  for (const key of ADMIN_SETTINGS_KEYS) {
    if (body[key] !== undefined) {
      ...
      if (key === 'smtp_pass' && val === ADMIN_SECRET_MASK) continue;
      if (key === 'smtp_pass') val = encrypt_api_key(val);
      if (key === 'admin_webhook_url' && val === ADMIN_SECRET_MASK) continue;
      if (key === 'admin_webhook_url' && val) val = maybe_encrypt_api_key(val) ?? val;
      if (key === 'admin_ntfy_token' && val === ADMIN_SECRET_MASK) continue;
      if (key === 'admin_ntfy_token' && val) val = maybe_encrypt_api_key(val) ?? val;
      db.prepare("INSERT OR REPLACE INTO app_settings (key, value) VALUES (?, ?)").run(key, val);
    }
  }

  const changedKeys = ADMIN_SETTINGS_KEYS.filter(k => body[k] !== undefined && !(k === 'smtp_pass' && String(body[k]) === ADMIN_SECRET_MASK));
...
export function setupMfa(userId: number, userEmail: string): { error?: string; status?: number; secret?: string; otpauth_url?: string; qrPromise?: Promise<string> } {
  ...
  try {
    ...
    otpauth_url = authenticator.keyuri(userEmail, 'TripTrace', secret);
  } catch (err) {
