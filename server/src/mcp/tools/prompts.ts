...
export function registerMcpPrompts(server: McpServer, _userId: number, isStaticToken = false): void {
    ...
    server.registerPrompt(
      ...
      async () => ({
        ...
        messages: [{
          ...
          content: {
            type: 'text' as const,
            text: 'WARNING: This MCP connection is authenticated with a static API token (trek_-. Static token authentication will be deprecated in a future version of TripTrace. Please inform the user that they should migrate to OAuth 2.1 by going to Settings ->Integrations ->MCP ->OAuth Clients in TripTrace and registering an OAuth 2.1 application for their MCP client.',
          },
        }],
      })
    );
  ...
  server.registerPrompt(
    ...
    async ({ tripId }) => {
      ...
      const memberList = [members?.owner, ...collaborators].filter(Boolean);
      const packingStats = packing ? { total: packing.total, packed: packing.checked } : { total: 0, packed: 0 };
      const budgetTotal = budget?.total || 0;
      const text = `Trip: ${trip?.title || 'Untitled'}${trip?.description ? `\n${trip.description}` : ''}
Dates: ${trip?.start_date || '?'} to ${trip?.end_date || '?'}
Members: ${memberList.length} (${memberList.map((m: any) => m.username || m.email).join(', ') || 'none'})
Days: ${days?.length || 0}
...
${days?.map((d: any, i: number) => `Day ${i + 1} (${d.date}): ${d.assignments?.length || 0} places${d.title ? ` - ${d.title}` : ''}`).join('\n') || 'No days yet'}`;
  ...
  if (isAddonEnabled(ADDON_IDS.BUDGET)) server.registerPrompt(
    ...
    async ({ tripId }) => {
      ...
      const byCategory = budgetItems.reduce((acc: Record<string, number>, item: any) => {
      ...
      const lines = Object.entries(byCategory)
        ...
        .join('\n');
      const memberCount = 1 + (summary.members?.collaborators?.length || 0);
      const perPerson = memberCount > 0 ? (total / memberCount).toFixed(2) : total.toFixed(2);
      return {
