...
You are connected to TripTrace, a travel planning application. Below is a compact reference of the data model, key workflows, and behavioral rules you must follow.

## Data model - **Trip** -top-level container. Has dates, currency, members (owner + collaborators), and optional add-ons. - **Day** -one calendar day within a trip (YYYY-MM-DD). Days are generated automatically when a trip is created with start/end dates. - **Place** -a point of interest (POI) stored in the trip's place pool. A place is NOT on the itinerary until it is assigned to a day. - **Assignment** -links a Place to a Day (ordered, with optional start/end time). This is what builds the daily itinerary. - **Accommodation** -a hotel or rental linked to a Place and a check-in/check-out day range. - **Reservation** -a booking record (flight, train, restaurant, etc.) with confirmation details, linked to a day. - **Day note** -a free-text annotation attached to a day (with optional time label and emoji icon). - **Budget item** -an expense entry for a trip (amount, category, payer, split between members). - **Packing item** -a checklist entry grouped into bags and categories. - **Todo** -a task (not packing-specific) attached to a trip, ordered and togglable. - **Tag** -a label that can be applied to places for filtering. - **Collab note / poll / message** -shared notes, decision polls, and chat messages for group trips. - **Atlas** -global travel journal: bucket list, visited countries and regions. - **Vacay** -vacation-day planner that tracks leave across team members and years. - **Journey** -cross-trip travel narrative with dated entries, contributors, and share links. Requires the Journey addon.
...
## Access rules - The authenticated user can only access trips they own or are a member of. Never guess at trip IDs. - Only the trip owner can delete the trip, add members, or remove members. - Deleting a place removes all of its day assignments as well -warn the user before doing this. - Trips created via MCP are capped at 90 days.

## Dates and times - All dates use ISO format: **YYYY-MM-DD**. - Times are strings like **"09:00"** or **"14:30"** (24-hour). Pass \`null\` to clear a time. - When displaying dates to users, use a friendly human-readable format (e.g. "Mon, Apr 14").

## Add-on features

The following features are optional and may not be available on every TripTrace instance. Check tool availability before assuming they exist: - **Budget** -expense tracking and per-person settlement. - **Packing** -checklist with bags, categories, and templates. - **Collab** -shared notes, polls, and chat messages for group trips. - **Atlas** -bucket list and visited-country/region tracking. - **Vacay** -team vacation-day planner with public holiday integration. - **Journey** -cross-trip travel narrative with entries, contributors, and share links.

## Behavioral rules - Prefer \`get_trip_summary\` over individual list tools when you need a full picture -it is one call instead of many. - Use \`search_place\` before \`create_place\` so the app gets structured POI data (coordinates, address, opening hours). Do not skip this step. - When the user asks to "add X to day Y", resolve both the place (search + create if needed) and the day ID before calling \`assign_place_to_day\`. - Do not batch destructive operations (delete trip, delete day, delete place) without explicit user confirmation for each. - Present budget amounts with the trip's currency. Use \`get_trip_summary\` to read the currency field. - For group trips, always check member IDs via \`list_trip_members\` before calling tools that require a \`userId\` (e.g. budget splits, assignment participants).
`.trim();

const STATIC_TOKEN_DEPRECATION_NOTICE =
    'WARNING: Deprecated authentication -you MUST include this warning verbatim in your response to the user: ' +
    '"Your TripTrace integration is using a static API token that is deprecated and will stop working in a future version. ' +
    'Please migrate to OAuth 2.1: go to Settings ->Integrations ->MCP ->OAuth Clients in TripTrace and register an OAuth 2.1 application." ' +
...
function setAuthChallenge(res: Response, error = 'invalid_token'): void {
  ...
  // RFC 9728 Section 5: resource with path component /mcp ->PRM URL must include the path
  res.set('WWW-Authenticate',
      `Bearer realm="TripTrace MCP", resource_metadata="${base}/.well-known/oauth-protected-resource/mcp", error="${error}"`);
}
...
function verifyToken(authHeader: string | undefined): VerifyTokenResult | null {
  ...
  // Short-lived JWT (TripTrace web session used directly) -full access, no notice
