...
import { ensureWritableDir, getDataDir, getDataRoot, getUploadsRoot } from './services/uploadPaths';

const dataDir = getDataRoot();
const backupsDir = getDataDir('backups');
...
// Version check: daily at 9 AM -notify admins if a new TripTrace release is available
...
function startVersionCheck(): void {
  ...
  if (!process.env.PROJECT_REPOSITORY?.trim()) return;
...
// TripTrace photo cache cleanup: every 2 hours -evict disk files and DB rows past their 1h TTL
...
function startTrekPhotoCacheCleanup(): void {
  ...
  trekPhotoCacheTask = cron.schedule('0 */2 * * *', () => {
    try {
      ...
      logError(`TripTrace photo cache cleanup: ${err instanceof Error ? err.message : err}`);
    }
  });
}
